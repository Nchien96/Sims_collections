...
const Donate = () => {
  ...
  const simAddress = "0xaCE7f7970BCb344fEA5387e4ecEd6EC0559A3C7D";
  ...
  const donate = async () => {
    try {
      const contract = await new web3.eth.Contract(donateAbi, donateAddress);
      const contractSim = await new web3.eth.Contract(simAbi, simAddress);
      let accounts = await window.ethereum.request({
        method: "eth_requestAccounts",
      });
      let donateAmount = Number(document.getElementById("inputField").value);
      let allowance = await contractSim.methods
      ...
      if (allowanceFromWei < donateAmount) {
        ...
        await contract.methods.donate(donateAmount).send({ from: accounts[0] });
      } else {
        await contract.methods.donate(donateAmount).send({ from: accounts[0] });
      }
    } catch (err) {
  ...
  const approve = async (donateAmount) => {
    try {
      const contract = await new web3.eth.Contract(simAbi, simAddress);
      let accounts = await window.ethereum.request({
        method: "eth_requestAccounts",
      });
